...
if __name__ == '__main__':
    ...
    cols = st.columns(3)
    if use_slider:
        Tcmd = cols[0].slider("Commanded Torque [Nm]", -1100, 1100, 300)
        Mz_cmd = cols[1].slider("Commanded Moment [Nm]", -2000, 2000, 0)
        steering = cols[2].slider("Steering Angle [deg]", -120, 120, 0)
    else:
        Tcmd = cols[0].number_input("Commanded Torque [Nm]", -1100, 1100, 300)
        Mz_cmd = cols[1].number_input("Commanded Moment [Nm]", -2000, 2000, 0)
        steering = cols[2].number_input("Steering Angle [deg]", -120, 120, 0)

    cols = st.columns(4)
    Tmax = np.array([0, 0, 0, 0])
    for i in range(4):
        if use_slider:
            Tmax[i] = cols[i].slider(f"{wheels[i]} [Nm]", 0, 275, 200)
        else:
            Tmax[i] = cols[i].number_input(f"{wheels[i]} [Nm]", 0, 275, 200)
    st.divider()
